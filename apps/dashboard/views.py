...
def home(request):
    ...
    # Charts Data
    # 1. Top 5 Workers
    top_workers_query = WorkEntry.objects.values('labor__name').annotate(earned=Sum('total_amount')).order_by('-earned')[:5]
    top_workers = {
    ...
    daily_prod_query = WorkEntry.objects.filter(work_date__gte=last_30_days).values('work_date').annotate(count=Sum('pieces')).order_by('work_date')
    daily_prod = {
    ...
    op_breakdown_query = WorkEntry.objects.values('operation__name').annotate(cost=Sum('total_amount')).order_by('-cost')
    op_breakdown = {
    ...
    context = {
        ...
        'recent_entries': WorkEntry.objects.select_related('lot', 'labor', 'operation').order_by('-created_at')[:10],
        'recent_payments': Payment.objects.select_related('labor').order_by('-created_at')[:5],
    }
